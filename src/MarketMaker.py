...
_METRICS_AGG_SCHEMA = {
    ...
    "best_bid_B": pl.Float64,
    "best_ask_B": pl.Float64,
    "EUR_quantity": pl.Float64,
    "USD_quantity": pl.Float64,
    "inventory_pct": pl.Float64,
    "hedge_regime": pl.String,
    ...
    def save_metrics(
        ...
        if self._step_count % MARKET_MAKER_AGGREGATION_STEPS == 0:
            ...
            row_agg = {
                ...
                "best_bid_A": b_price,
                "best_ask_A": a_price,
                "best_bid_B": float(order_book_B.best_bid[0]) if order_book_B.best_bid[0] is not None else float("nan"),
                "best_ask_B": float(order_book_B.best_ask[0]) if order_book_B.best_ask[0] is not None else float("nan"),
                "EUR_quantity": float(self.EUR_quantity),
                "USD_quantity": float(self.USD_quantity),
                "inventory_pct": float(inv_pct),
                "hedge_regime": str(regime),
